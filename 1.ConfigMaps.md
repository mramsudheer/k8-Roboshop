...
&emsp;•&emsp;A ConfigMap is a Kubernetes object used to store non-confidential configuration data in key-value pairs.<br>
&emsp;•&emsp;ConfigMaps help you separate config values from your code and container images. These values can be strings or Base64-encoded binary data.<br>
&emsp;•&emsp;ConfigMaps are essential for managing configuration settings separately from the application code, making your applications more flexible and portable.<br>
&emsp;•&emsp;After creating a ConfigMap, the ConfigMap is then stored in Kubernetes cluster. Pods can access the ConfigMap's values as environment variables, command line arguments, or as part of the filesystem volumes.<br><br>
...
To see which namespace your Linux terminal is currently tuned into, run this command:<br>
```hcl
    kubectl config view --minify --output 'jsonpath={..namespace}'
```
&emsp;•&emsp;If the output is blank, your Linux terminal is pointing to the `default` namespace.<br>
&emsp;•&emsp;Any command you run without `-n` will execute inside whatever namespace this command prints out.<br><br>
**Switch your active namespace permanently (The Native Way)** <br>
If you want to stop typing `-n my-namespace` every time you run a command on Linux, you can tell your `kubeconfig` to permanently target `my-namespace` for your current context:<br>
```hcl
    kubectl config set-context --current --namespace=my-namespace
```
*What happens now:* If you run kubectl create configmap `my-config --from-file=app.properties` right after this, Linux automatically routes it to `my-namespace`.<br><br    >
**Use `kubens` (Highly Recommended for Linux)** If you frequently switch between your three namespaces on Linux, you should install a tiny, incredibly popular open-source tool called kubens.<br>
&emsp;•&emsp;To switch namespaces instantly:<br>
```hcl
    kubens my-namespace
```
...
```hcl
    kubectl create configmap my-config --from-literal=db_host=database.example.com --from-literal=db_port=5432 -n my-namespace
```
...
```hcl
    kubectl create configmap my-config --from-file=app.properties -n my-namespace
```
...
```hcl
    kubectl create configmap my-config --from-file=app.properties --from-file=log.properties -n my-namespace
```
...
```hcl
    kubectl create configmap my-config --from-file=config-dir/ -n my-namespace
```
...
&emsp;&emsp;You might need to combine multiple ConfigMaps for a single application. Kubernetes allows you to use multiple ConfigMaps within the same pod.<br><br>
...
```
<br>NOTE: If `namespace` is mentioned in YAML `metadata: namespace: my-namespace` session, then appending `-n my-namespace` in `kubectl edit` or `apply` command is not required <br>
Alternatively, you can apply changes from a modified YAML file.<br>
```hcl
    kubectl apply -f updated-configmap.yaml -n my-namespace
```
**Using ConfigMaps with Deployments**<br>
In a production environment, you typically use ConfigMaps with Kubernetes Deployments.<br><br>
...
```hcl
    kubectl create configmap db-config --from-file=db.properties -n my-namespace
    kubectl create configmap api-config --from-file=api.properties -n my-namespace
```
...
```hcl
    kubectl create configmap config-dev --from-file=dev.properties -n my-namespace
    kubectl create configmap config-prod --from-file=prod.properties -n my-namespace
```
...
```hcl
    kubectl create configmap app-config --from-file=app.properties -n my-namespace
    kubectl create configmap log-config --from-file=log.properties -n my-namespace
```
&emsp;7.&emsp; **Keep ConfigMaps Small**<br>
Avoid putting too much data into a single ConfigMap. Split large configurations into multiple ConfigMaps to keep them manageable.<br><br>
